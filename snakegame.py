...
pygame.display.set_caption('Snake') #NOME QUE APARECE NA TELA DO JOGO
...
while not game_over: #ENQUANTO NAO DER GAME OVER VAI RODAR:
    ...
    for event in pygame.event.get():  #TIPOS DE POSSIVEIS EVENTOS NO GAME:
        ...
        if event.type == KEYDOWN: #EVENTOS EM QUE ALGUMA TECLA É PRESSIONADA:
            ...
            if event.key == K_DOWN and my_direction != UP: # SE APERTAR *BAIXO* E A SNAKE NÃO ESTIVER ANDANDO PRA CIMA:
                my_direction = DOWN # A SNAKE IRA PARA BAIXO
                pygame.mixer.Sound.play(direction_sound)
            if event.key == K_LEFT and my_direction != RIGHT: # SE APERTAR *ESQUERDA* E A SNAKE NÃO ESTIVER ANDANDO PRA DIREITA:
                my_direction = LEFT # A SNAKE IRA PARA ESQUERDA
                pygame.mixer.Sound.play(direction_sound)
            if event.key == K_RIGHT and my_direction != LEFT: # SE APERTAR *DIREITA* E A SNAKE NÃO ESTIVER ANDANDO PRA ESQUERDA:
